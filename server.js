...
<head>
  ...
  <title>Admin</title>
  <style>
    :root{--fg:#f0ece4;--muted:#9a948e;--accent:#c8a55c;--bg:#070707;}
    html,body{height:100%;}
    body{margin:0;background:var(--bg);color:var(--fg);font-family:system-ui,-apple-system,Segoe UI,Roboto,Arial,sans-serif;overflow:hidden;}
    #bg{position:fixed;inset:0;width:100%;height:100%;display:block;}
    .vignette{position:fixed;inset:0;pointer-events:none;background:radial-gradient(1200px 800px at 50% 40%, rgba(0,0,0,0) 0%, rgba(0,0,0,0.35) 55%, rgba(0,0,0,0.75) 100%);} 
    .topGlow{position:fixed;inset:-40% -20% auto -20%;height:70%;background:conic-gradient(from 180deg at 50% 50%, rgba(200,165,92,0.18), rgba(92,184,138,0.12), rgba(138,180,248,0.10), rgba(200,165,92,0.18));filter:blur(30px);opacity:0.9;mix-blend-mode:screen;}
    .wrap{position:fixed;inset:0;display:grid;place-items:center;padding:28px;}
    .card{width:min(920px, 92vw);border:1px solid rgba(255,255,255,0.12);border-radius:22px;background:rgba(15,15,15,0.55);backdrop-filter:blur(10px);box-shadow:0 24px 90px rgba(0,0,0,0.55);padding:22px;}
    .badge{display:inline-flex;align-items:center;gap:10px;padding:8px 12px;border-radius:999px;border:1px solid rgba(255,255,255,0.12);background:rgba(255,255,255,0.03);color:var(--muted);font-size:12px;font-weight:700;letter-spacing:0.02em;}
    h1{margin:14px 0 10px;font-size:28px;letter-spacing:-0.02em;}
    p{margin:0 0 16px;color:var(--muted);line-height:1.55;}
    .row{display:flex;gap:12px;flex-wrap:wrap;align-items:center;margin-top:14px;}
    a.btn{display:inline-flex;align-items:center;gap:10px;padding:10px 14px;border-radius:999px;border:1px solid rgba(255,255,255,0.14);background:rgba(255,255,255,0.06);color:var(--fg);text-decoration:none;font-weight:800;}
    a.btn.primary{background:rgba(200,165,92,0.18);border-color:rgba(200,165,92,0.32);}
    .tiny{font-size:12px;color:rgba(240,236,228,0.55);margin-top:14px;}
    .glitch{position:relative;display:inline-block;}
    .glitch::before,.glitch::after{content:attr(data-text);position:absolute;left:0;top:0;opacity:0.75;}
    .glitch::before{transform:translate(1px,0);color:rgba(138,180,248,0.95);clip-path:polygon(0 0,100% 0,100% 38%,0 38%);}
    .glitch::after{transform:translate(-1px,0);color:rgba(92,184,138,0.95);clip-path:polygon(0 62%,100% 62%,100% 100%,0 100%);}
...
<body>
  ...
  <div class="wrap">
    <div class="card" role="dialog" aria-label="Area admin">
      <span class="badge">Area Admin</span>
      <h1 class="glitch" data-text="Nice try.">Nice try.</h1>
      <p>Questa pagina è disponibile solo in <strong>locale</strong>.</p>
      <p>Se cercavi il CV pubblico, lo trovi qui:</p>
      <div class="row">
        <a class="btn primary" href="/cv">Apri CV</a>
  ...
  <script>
    (function(){
      ...
      const glyphs = '01 NICOLA CV ADMIN ACCESS DENIED 🔒'.split('');
      ...
      function step(){
        ...
        for(let i=0;i<cols;i++){
          ...
          ctx.fillStyle = 'rgba(200,165,92,' + alpha + ')';
          ctx.fillText(ch, x, y);
          drops[i] = y + 16 + Math.random()*10;
          if (drops[i] > h + 50 && Math.random() > 0.975) drops[i] = -Math.random()*200;
        }
